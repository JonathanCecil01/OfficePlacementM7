...
def run_model(products, color_dict):    #running saved model from the ipnb 
    df = pd.read_csv('OfficeSpaceUtil/classified_data.csv')
    ...
    model = tf.keras.models.load_model('OfficeSpaceUtil/count_included_nn')           #loading the model 
    outputs = model.predict(inputs)                                 #prediction
    outputs = outputs.tolist()                                      #converting to list
    output_labels= []
    ...
    for i in range(len(product_label)):
        for product in products:
            if product.item_no == product_label[i]:
                product.predicted_landmark_id = output_labels[i]
