...
def read_rename():
    rawData = pd.read_csv('OfficeSpaceUtil/rawData.csv')
...
def combine_rows(group):
    ...
    for _, row in group.iterrows():
        ...
        new_row = pd.Series([row['locationId'], row['productId'], row['EPC'], new_rssi, row['minRSSI'], row['maxRSSI'], count_sum, 0, 30, 0, row['startTimeStamp'], timestamp_new, row['isLocation'], row['actualLandmark']], index=group.columns)
        ...
        #for key in checkDict.keys():
         for key in check1:
            if location_ids[i] == key:    
                ...
                outDict['locationCount'+location_ids[i]] = outDict['locationCount'+location_ids[i]].astype(np.int64)
...
def preprocess():
    ...
    consColumnNames = ['productId', 'EPC', 'RSSI', 'count', 'endTimeStamp', 'actualLandmark']
    ...
    consData['actualLandmark'] = combinedRawDataP['actualLandmark']
    ...
    reordercColumns.remove('actualLandmark')
    reordercColumns.append('actualLandmark')

    # Reorder the DataFrame columns
    classified_df = classified_df[reordercColumns]
    print(classified_df)
    classified_df.to_csv('OfficeSpaceUtil/classified_data.csv', index=False)
    return
